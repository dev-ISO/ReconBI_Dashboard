...
import { ExportImageDialog, type ExportImageRequest } from './ExportImageDialog';
...
export function DashboardView({
  ...
  /**
   * "Export image…": opens the PNG-export dialog for a tile. Entire-tile
   * capture works for every renderer (HTML table/KPI included); the bare-plot
   * option needs a chart SVG, so the dialog is told whether one exists. The
   * tile's DOM is found through DashboardChartTile's data-rcd-tile anchor.
   */
  const [exportImage, setExportImage] = useState<ExportImageRequest | null>(null);
  const openExportImage = (tileId: string, chart: ChartSpec) =>
    setExportImage({
      tileId,
      title: chart.title,
      hasPlotSvg: chart.type !== 'table' && chart.type !== 'kpi',
    });
  const resolveTileRoot = (tileId: string): HTMLElement | null =>
    document.querySelector<HTMLElement>(`[data-rcd-tile="${CSS.escape(tileId)}"]`);
  ...
  return (
    <div ref={rootRef} className="flex h-full min-h-0 flex-col bg-rcd-bg">
        ...
        createPortal(
          <div className="rcd-root bg-transparent">
            <PointContextMenu
              ...
              onExportImage={() => openExportImage(pointMenu.tileId, pointMenu.chart)}
        ...
        createPortal(
          <div className="rcd-root bg-transparent">
            <PointContextMenu
              ...
              onExportImage={() => openExportImage(tileMenu.tileId, tileMenu.chart)}
      ...
      <ExportImageDialog
        request={exportImage}
        resolveTileRoot={resolveTileRoot}
